...
static const TimproRdmaFshdlAddr S_G_IM_PRO_FSHDL_ADDR[E_IM_PRO_UNIT_NUM_MAX]
...
static const T_IM_PRO_SRO_FSHDL_INFO	S_G_IM_PRO_FSHDL_STATUS_TBL[E_IM_PRO_UNIT_NUM_MAX]
                                    	                            		[ImproSrofshdl_D_IM_PRO_SRO_FSHDL_CH_NUM] = {
	// E_IM_PRO_UNIT_NUM_1
	{
...
INT32 impro_srofshdl_start( E_IM_PRO_UNIT_NUM unitNo, EimproFshdlCh ch )
{
	...
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHTRG.bit.ffshtrg = D_IM_PRO_TRG_START;
	// Dd_Top_Stop_Clock
...
INT32 impro_srofshdl_stop( E_IM_PRO_UNIT_NUM unitNo, EimproFshdlCh ch, UCHAR force )
{
	...
	if(force == 0) {
		// stop
		S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHTRG.bit.ffshtrg = D_IM_PRO_TRG_FRAME_STOP;
	}
	else {
		// force stop
		S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHTRG.bit.ffshtrg = D_IM_PRO_TRG_FORCE_STOP;
	}
	// Dd_Top_Stop_Clock
	im_pro_off_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
...
INT32 impro_srofshdl_ctrl( E_IM_PRO_UNIT_NUM unitNo, EimproFshdlCh ch, TimproFshdlCommonCtrl* fshdlCtrl )
{
	...
	// Dd_Top_Start_Clock
	im_pro_on_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHCTL.bit.FFSHPTMD		= fshdlCtrl->montageMode;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHUL1.bit.FFSHULRR		= fshdlCtrl->clipR;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHUL1.bit.FFSHULGR		= fshdlCtrl->clipGr;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHUL2.bit.FFSHULGB		= fshdlCtrl->clipGb;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHUL2.bit.FFSHULBB		= fshdlCtrl->clipB;
	// Dd_Top_Stop_Clock
	im_pro_off_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
...
INT32 impro_srofshdl_set_blend( E_IM_PRO_UNIT_NUM unitNo, EimproFshdlCh ch, TimproFshdlBlendCtrl* blendCtrl )
{
	...
	// Dd_Top_Start_Clock
	im_pro_on_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
	im_pro_set_reg_signed( S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHBC,
...
{
#ifdef CO_PARAM_CHECK
	if( ( paenTrg == 0 ) && ( S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHTRG.bit.ffshtrg
	...
	// Dd_Top_Start_Clock
	im_pro_on_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHPAEN.bit.FFSHPAEN	= paenTrg;
	// Dd_Top_Stop_Clock
	im_pro_off_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
...
@retval		D_IM_PRO_INPUT_PARAM_ERROR		: Setting NG
*/
INT32 impro_srofshdl_set_area( E_IM_PRO_UNIT_NUM unitNo, EimproFshdlCh ch, T_IM_PRO_AREA_INFO* fshdlArea )
{
#ifdef CO_PARAM_CHECK
	if (fshdlArea == NULL){
	...
	// Dd_Top_Start_Clock
	im_pro_on_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHV.bit.FFSHV			= fshdlArea->posY;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHH.bit.FFSHH			= fshdlArea->posX;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHVW.bit.FFSHVW		= fshdlArea->lines;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHHW.bit.FFSHHW		= fshdlArea->width;
	// Dd_Top_Stop_Clock
	im_pro_off_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
...
"impro_srofshdl_set_frame : startPosV" ) == FALSE ) {
	...
	// Dd_Top_Start_Clock
	im_pro_on_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHGFMT.bit.FFSHGFMT	= fshdlCtrl->selectFormat;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBPR.bit.ffshsubprH	= fshdlCtrl->frameHorizontalSize;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBPR.bit.ffshsubprV	= fshdlCtrl->frameVorizontalSize;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBSP.bit.ffshsubspH	= fshdlCtrl->horizontalPixel;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBSP.bit.ffshsubspV	= fshdlCtrl->verticalPixel;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBDR.bit.FFSHSUBDRM	= fshdlCtrl->InverseBase;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBDR.bit.FFSHSUBDRE	= fshdlCtrl->InverseExponent;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBST.bit.FFSHSUBSTH	= fshdlCtrl->startPosH;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBST.bit.FFSHSUBSTV	= fshdlCtrl->startPosV;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHHWPH.bit.FFSHHWPH	= fshdlCtrl->montagePosH;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHCCTL.bit.FFSHCEN		= fshdlCtrl->satCorrectionEnabled;
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHLP1.bit.FFSHLPRR		= fshdlCtrl->satSlopeGain[0];
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHLP1.bit.FFSHLPGR		= fshdlCtrl->satSlopeGain[1];
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHLP2.bit.FFSHLPGB		= fshdlCtrl->satSlopeGain[2];
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHLP2.bit.FFSHLPBB		= fshdlCtrl->satSlopeGain[3];
	// Dd_Top_Stop_Clock
	im_pro_off_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
...
INT32 impro_srofshdl_set_coeff_read_line_wait( E_IM_PRO_UNIT_NUM unitNo, EimproFshdlCh ch, USHORT reqIntervalClk )
{
	...
	else if( reqIntervalClk >= ((S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHHW.bit.FFSHHW
			- (4*S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHSUBPR.bit.ffshsubprH))-10)/3 ) {
	...
	// Dd_Top_Start_Clock
	im_pro_on_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
	S_G_IM_PRO_FSHDL_STATUS_TBL[unitNo][ch].reg_ptr->FFSHWRIC.bit.FFSHWRIC	= reqIntervalClk;
	// Dd_Top_Stop_Clock
	im_pro_off_pclk( unitNo, E_IM_PRO_CLK_BLOCK_TYPE_SRO );
