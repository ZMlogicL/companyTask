...
INT32 Im_DISP_Start(UINT32 hdmi_layer, UINT32 lcd_mipi_layer, BYTE write_channel)
{
	...
	// HDMI(Common block 0)
	if((hdmi_layer & E_IM_DISP_SEL_LAYER_ALL) != E_IM_DISP_SEL_LAYER_NONE) {
		ret = im_disp_set_trg(im_disp_new(), E_IM_DISP_HDMI, hdmi_layer, write_channel, E_IM_DISP_TRG_WRITE_ACT);
	}

	// LCD or MIPI-DSI(Common block 1)
	if((lcd_mipi_layer & E_IM_DISP_SEL_LAYER_ALL) != E_IM_DISP_SEL_LAYER_NONE) {
		ret = im_disp_set_trg(im_disp_new(), E_IM_DISP_LCD_MIPI, lcd_mipi_layer, write_channel, E_IM_DISP_TRG_WRITE_ACT);
	}
...
INT32 Im_DISP_Stop(UINT32 hdmi_layer, UINT32 lcd_mipi_layer, BYTE write_channel, E_IM_DISP_STOP_TYPE stop_type)
{
	...
	if( hdmi_layer & (~E_IM_DISP_SEL_LAYER_NONE)) {
		...
		ret = im_disp_set_trg(im_disp_new(), E_IM_DISP_HDMI, hdmi_layer, write_channel, set_type);
	}
	...
	if( lcd_mipi_layer & (~E_IM_DISP_SEL_LAYER_NONE)) {
		...
		ret = im_disp_set_trg(im_disp_new(), E_IM_DISP_LCD_MIPI, lcd_mipi_layer, write_channel, set_type);
	}
...
INT32 Im_DISP_Wait_Stop(UINT32 hdmi_layer, UINT32 lcd_mipi_layer)
{
	...
	// common block 0
	if ((hdmi_layer & E_IM_DISP_SEL_LAYER_ALL) != E_IM_DISP_SEL_LAYER_NONE) {
		ret = im_disp_wait_trg_stop_start(im_disp_new(), E_IM_DISP_HDMI, hdmi_layer, E_IM_DISP_TRG_READ_NO_ACT);
		if (ret != D_DDIM_OK){
	...
	// common block 1
	if ((lcd_mipi_layer & E_IM_DISP_SEL_LAYER_ALL) != E_IM_DISP_SEL_LAYER_NONE) {
		ret = im_disp_wait_trg_stop_start(im_disp_new(), E_IM_DISP_LCD_MIPI, lcd_mipi_layer, E_IM_DISP_TRG_READ_NO_ACT);
		if (ret != D_DDIM_OK){
			Ddim_Print(("E:Im_DISP_Wait_Stop: Time-out occur when waiting the block stop\n"));
		}
	}
