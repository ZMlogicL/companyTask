...
@remarks	@ref im_disp2_start or @ref im_disp2_stop can be called 1 time only at same frame.<br>
...
@retval	D_IM_DISP_ACCESS_ERR			TRG cannot be set up.
@retval	D_IM_DISP_TIMEOUT				The channel cannot stop within 33ms.
@remarks	If it's necessary, call the @ref im_disp2_wait_stop to wait the channel stopping <br>
			when the @ref E_IM_DISP_STOP_TYPE_WITH_WAIT was used.<br>
			@ref im_disp2_start or @ref im_disp2_stop can be called 1 time only at same frame.<br>
...
@param[in]	interruption_select	Bit map to select wait item. <br>
							<ul><li>@ref ImDisp_E_IM_DISP_INTERRUPTION_SELECT_VE
								<li>@ref ImDisp_E_IM_DISP_INTERRUPTION_SELECT_VE2
								<li>@ref ImDisp_E_IM_DISP_CORRECT_SELECT_ALL_VE</ul>
...
*/
 extern  VOID im_disp2_int_handler(void);
