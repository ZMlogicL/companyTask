...
K_TYPE_DEFINE_WITH_PRIVATE(CtImEla5, ct_im_ela5);
#define CT_IM_ELA5_GET_PRIVATE(o)(K_OBJECT_GET_PRIVATE ((o),CtImEla5Private,CT_TYPE_IM_ELA5))

struct _CtImEla5Private
{
	INT32 result;
	CtImEla* ela;
	ImElaReg* elareg;
...
static void ct_im_ela5_constructor(CtImEla5 *self) 
{
	CtImEla5Private *priv = CT_IM_ELA5_GET_PRIVATE(self);
...
static void ct_im_ela5_destructor(CtImEla5 *self) 
{
	CtImEla5Private *priv = CT_IM_ELA5_GET_PRIVATE(self);
	if(priv->ela){
...
void ct_im_ela5_2_18(CtImEla5 *self)
{
 ...
 ctrlEla.ctrl_eirch.in_bayer_total_hsize = 1280;
 ctrlEla.ctrl_eirch.in_bayer_hsize = 640;
 ctrlEla.ctrl_eirch.in_bayer_vline = 480;
 ...
 ctrlEla.ctrl_byr.in_data_shift_mode = ImEla_D_IM_ELA_BIT_SHIFT_MODE_L4;
 ctrlEla.ctrl_byr.out_data_type = ImEla_D_IM_ELA_DATATYP_12BIT;
 ctrlEla.ctrl_byr.out_data_shift_mode = ImEla_D_IM_ELA_BIT_SHIFT_MODE_R4;
...
*/
 ...
 ctrlEla.ctrl_byr.out_data_type = ImEla_D_IM_ELA_DATATYP_12BIT;
 ctrlEla.ctrl_byr.out_data_shift_mode = ImEla_D_IM_ELA_BIT_SHIFT_MODE_NONE;
 ctrlEla.ctrl_byr.deknee_enable = ImEla_D_IM_ELA_DISABLE;
 ctrlEla.ctrl_byr.ob_zsf_enable = ImEla_D_IM_ELA_DISABLE;
 ...
 ctrlEla.ctrl_common.bayer_start_pixel_type = ImEla_D_IM_ELA_ORG_R;
	...
	for(i = 0; i < ImElaReg_D_IM_ELA_STAGE_MAX; i++){
		multipleExecInfo.core[i]->optical_zero_level_shift = 0x07FF;
		multipleExecInfo.core[i]->limit.noise_plus_limit = 0x0277;
		multipleExecInfo.core[i]->limit.noise_minus_limit = 0x0277;
	}
...
*/
 ...
 ctrlEla.ob_correction.ob_rr = 0x7FF;
 ctrlEla.ob_correction.ob_bb = 0x7FF;
 ctrlEla.ob_correction.ob_gr = 0x7FF;
 ctrlEla.ob_correction.ob_gb = 0x7FF;
...
{
    CtImEla5 *self = k_object_new_with_private(CT_TYPE_IM_ELA5, sizeof(CtImEla5Private));
    return self;
}
