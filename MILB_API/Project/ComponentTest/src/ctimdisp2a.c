...
K_TYPE_DEFINE_DERIVED_WITH_PRIVATE(CtImDisp2a, ct_im_disp2a, IM_TYPE_DISP2_PARENT)
#define CT_IM_DISP2A_GET_PRIVATE(o) (K_OBJECT_GET_PRIVATE ((o), CtImDisp2aPrivate, CT_TYPE_IM_DISP2A))
...
static void disp2aDoMain_od(ImDisp2Parent *parent, kint32 argc, char **argv);
 ...
 * IMPL
 * */
static void ct_im_disp2a_constructor(CtImDisp2a *self)
{
...
static void ct_im_disp2a_destructor(CtImDisp2a *self)
{
}
...
static void disp2aDoMain_od(ImDisp2Parent *parent, kint32 argc, char **argv)
{
	...
	if (strcmp((kchar*) argv[1], "CtrlOUTs") == 0)
	{
		//Im_DISP_Ctrl_Output
...
//				}
			...
			else if (strcmp((kchar*) argv[4], "40") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB888_8BIT_1;
			}
			else if (strcmp((kchar*) argv[4], "41") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_AVERAGE;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB888_8BIT_2;
			}
			else if (strcmp((kchar*) argv[4], "42") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_RIGHT;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB888_8BIT_3;
			}
			else if (strcmp((kchar*) argv[4], "43") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_RIGHT;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB888_16BIT_1;
			}
			else if (strcmp((kchar*) argv[4], "44") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_AVERAGE;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB888_16BIT_2;
			}
			else if (strcmp((kchar*) argv[4], "45") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB565_8BIT;
			}
			else if (strcmp((kchar*) argv[4], "46") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB565_16BIT;
			}
			else if (strcmp((kchar*) argv[4], "47") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_RGB666_18BIT;
			}
			else if (strcmp((kchar*) argv[4], "48") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_YCC444_24BIT;
			}
			else if (strcmp((kchar*) argv[4], "49") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_DEEP_YCC444_30BIT;
			}
			else if (strcmp((kchar*) argv[4], "50") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_DEEP_YCC444_36BIT;
			}
			else if (strcmp((kchar*) argv[4], "51") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_DUAL_YCC444_12BIT;
			}
			else if (strcmp((kchar*) argv[4], "52") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_GENERAL_8BIT;
			}
			else if (strcmp((kchar*) argv[4], "53") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_GENERAL_16BIT;
			}
			else if (strcmp((kchar*) argv[4], "54") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_GENERAL_10BIT_1;
			}
			else if (strcmp((kchar*) argv[4], "55") == 0)
			{
				// ctoh error
				outCtrl.doctl2.bit.CTOH = D_IM_DISP_CTOH_FILTER;
				outCtrlTrgLimit.domd.bit.MODE = D_IM_DISP_MODE_GENERAL_10BIT_2;
			...
			else if (strcmp((kchar*) argv[4], "66") == 0)
			{
			...
			Disp2bNewParams mParams = {(kpointer)&outCtrlTrgLimit, (kpointer)&outCtrl};
			ImDisp2Parent *imDisp2Parent = im_disp2_group_create_im_disp(disp2Group,
					ImDisp2Group_DO_IM_DISP_TEST_B, (KData)&mParams);
			if(imDisp2Parent){
				im_disp2_parent_do_main(imDisp2Parent, argc, argv);
				k_object_unref(imDisp2Parent);
				imDisp2Parent = NULL;
			}
//			ct_im_disp2b_do_main(argc, argv, &outCtrlTrgLimit, &outCtrl);
	...
	else if (strcmp((kchar*) argv[1], "CtrlOUTg") == 0)
	{
...
static void getCtrlOutLayer(BYTE getDataType, BYTE block)
{
	...
	kint32 error = Im_DISP_Get_Ctrl_Output(blockNum, pCtrlTrgLimit, pCtrl);
	if(error != D_DDIM_OK){
	...
	if(pCtrlTrgLimit){
		...
		Ddim_Print(("tgkst          = %08X\n",(kuint32)ctrlTrgLimit.tgkst));
		Ddim_Print(("toctl          = %08X\n",(kuint32)ctrlTrgLimit.toctl.word));
		// Get RPGCTL/POLSEL/TSL
		Ddim_Print(("rpgctl         = %08X\n",ctrlTrgLimit.rpgctl));
		Ddim_Print(("polsel         = %08X\n",(kuint32)ctrlTrgLimit.polsel.word));
		Ddim_Print(("tsl            = %08X\n",(kuint32)ctrlTrgLimit.tsl));
		// Get VCYC/HCYC/VPW/HPW/VBLK/HBLK/VDLY/HDLY/OVSIZE/OHSIZE
		Ddim_Print(("vcyc           = %08X\n",(kuint32)ctrlTrgLimit.vcyc.word));
		Ddim_Print(("hcyc           = %08X\n",(kuint32)ctrlTrgLimit.hcyc));
		Ddim_Print(("ovpw           = %08X\n",(kuint32)ctrlTrgLimit.ovpw.word));
		Ddim_Print(("hpw            = %08X\n",(kuint32)ctrlTrgLimit.hpw));
		Ddim_Print(("vblk           = %08X\n",(kuint32)ctrlTrgLimit.vblk.word));
		Ddim_Print(("hblk           = %08X\n",(kuint32)ctrlTrgLimit.hblk));
		Ddim_Print(("vdly           = %08X\n",(kuint32)ctrlTrgLimit.vdly));
		Ddim_Print(("hdly           = %08X\n",(kuint32)ctrlTrgLimit.hdly));
		Ddim_Print(("ovsize         = %08X\n",(kuint32)ctrlTrgLimit.ovsize));
		Ddim_Print(("ohsize         = %08X\n",(kuint32)ctrlTrgLimit.ohsize));
		// Get VRFCTL/HRFCTL
		Ddim_Print(("vrfctl         = %08X\n",(kuint32)ctrlTrgLimit.vrfctl));
		Ddim_Print(("hrfctl         = %08X\n",(kuint32)ctrlTrgLimit.hrfctl));
		// Get DOMD
		Ddim_Print(("toctl          = %08X\n",(kuint32)ctrlTrgLimit.domd.word));
	...
	if(pCtrl){
		...
		Ddim_Print(("tblaset_gmtsl   = %02X\n", (kuint32)ctrl.tblaset_gmtsl));
		Ddim_Print(("tblaset_ysaten  = %02X\n", ctrl.tblaset_ysaten));
		Ddim_Print(("tblaset_ysatsl  = %02X\n", (kuint32)ctrl.tblaset_ysatsl));

		Ddim_Print(("--- ffdsta ---\n"));
		for(loop = D_IM_DISP_FACE_FRAME_COUNT - 1; loop >= 0 ; loop--) {
			Ddim_Print(("%02d : %08X\n", loop, (kuint32)ctrl.face.param[loop].ffdsta.word));
		}

		Ddim_Print(("--- ffsize ---\n"));
		for(loop = D_IM_DISP_FACE_FRAME_COUNT - 1; loop >= 0 ; loop--) {
			Ddim_Print(("%02d : %08X\n", loop, (kuint32)ctrl.face.param[loop].ffsize.word));
		}

		Ddim_Print(("--- ffwidth ---\n"));
		for(loop = D_IM_DISP_FACE_FRAME_COUNT - 1; loop >= 0 ; loop--) {
			Ddim_Print(("%02d : %08X\n", loop, (kuint32)ctrl.face.param[loop].ffwidth.word));
		}

		Ddim_Print(("--- ffclr ---\n"));
		for(loop = D_IM_DISP_FACE_FRAME_COUNT - 1; loop >= 0 ; loop--) {
			Ddim_Print(("%02d : %08X\n", loop, (kuint32)ctrl.face.param[loop].ffclr.word));
		}
		...
		Ddim_Print(("--- ffdispen ---\n"));
		Ddim_Print(("face.msff   = %02X\n", (kuint32)ctrl.face.msff));

		Ddim_Print(("--- VP_CALLBACK[0~8] ---\n"));
		ct_im_disp3a_output_word_table((kuint32*)ctrl.int_callback, E_IM_DISP_INT_CB_MAX);	/* pgr0672 */
	}
}
...
{
	CtImDisp2a *self = (CtImDisp2a *) k_object_new_with_private(CT_TYPE_IM_DISP2A,sizeof(CtImDisp2aPrivate));
	return self;
}
