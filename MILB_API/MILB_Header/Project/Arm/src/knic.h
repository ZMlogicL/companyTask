...
struct _IoAddrctrl
{
    unsigned char dmy0000000b[0x000C - 0x0000]; // 1D10_(0000 - 000B) Reserved
    NicSecurity1 security1;                      // 1D10_(000C - 000F) Slave 1 security setting Register
    unsigned char dmy00100fff[0x1000 - 0x0010]; // 1D10_(0010 - 0FFF) Reserved
};

/* Slave Interface register                         */
struct _IoAsib
{
    unsigned char dmy2000203f[0x2040 - 0x2000]; // 1D14_(2000 - 203F) Reserved
    NicTidemark wrTidemark;                    // 1D14_(2040 - 2043) Tidemark Register
    unsigned char dmy204420ff[0x2100 - 0x2044]; // 1D14_(2044 - 20FF) Reserved
    ...
    unsigned char dmy210c2fff[0x3000 - 0x210C]; // 1D14_(210C - 2FFF) Reserved
};
